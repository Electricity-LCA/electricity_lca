...
async def get_electricity_generation_df(date_start, region_code: str, generation_type_id: int,engine) -> pd.DataFrame:
    ...
    with session_obj() as session:
        ...
        # if isinstance(ElectricityGenerationSchema.validate(df), (SchemaError, SchemaErrors)):
        #     logging.error('Schema error in electricity generation data returned from database')
        #     raise ServerError('Schema error in electricity generation data returned from database')
    return df
